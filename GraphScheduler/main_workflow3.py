...
if __name__ == '__main__':
    ...
    while True:
        ...
        break_flag = True
        for node_id, info in dag.nodes.items():
            if not info["async"] and not info["merged_try"]:
                break_flag = False
        if break_flag:
            break
        '''进行一次融合尝试'''
        ...
        after_cost = 0
        for node_id, info in dag.nodes.items():
            before_cost += calculate_cost(info["total_time"], info["memory"], num_calls=20)
        for node_id, info in merged_dag.nodes.items():
            after_cost += calculate_cost(info["total_time"], info["memory"], num_calls=20)
        if before_cost >= after_cost:
            dag = merged_dag
        else:
            for node_id, info in dag.nodes.items():
                if node_id==longest_sync_function:
                    info["merged_try"] = True
        count += 1
