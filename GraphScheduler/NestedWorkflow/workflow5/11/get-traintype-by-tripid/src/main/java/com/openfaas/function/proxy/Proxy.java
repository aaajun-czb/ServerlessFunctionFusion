...
public class Proxy {
    ...
    public static void main(String[] args) {
        ...
        // 检查命令行参数
        if (args.length < 1) {
            System.out.println("Usage: java com.openfaas.function.proxy.Proxy <tripId>");
            return;
        }
        ...
        // 构建 tripID 对象
        TripId tripIdObj = new TripId(tripId);

        // 创建Handler实例
        Handler handler = new Handler();

        // 调用 handle 方法
        Map<String, Object> response = handler.Handle(tripIdObj);
